...
export class Outcome {
    ...
    getDescription(): string {
        if (this.action.stat) {
            return `###(${this.action.stat}) ${this.action.description}###\n#${this.getDieEmoji(this.dieResult1)}${this.dieResult1} + ${this.getDieEmoji(this.dieResult2)}${this.dieResult2}${this.action.difficultyModifier >= 0 ? ' + ' : ' - '}${Math.abs(this.action.difficultyModifier)}<sup><sup>difficulty</sup></sup>${this.action.skillModifier > 0 ? ` + ${this.action.skillModifier}<sup><sup>skill</sup></sup>` : ''} = ${this.total} (${this.result})#`
        } else {
